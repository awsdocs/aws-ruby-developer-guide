# Sending Events to Amazon CloudWatch Events<a name="cw-example-send-events"></a>

The following code example shows how to create and trigger a rule in Amazon CloudWatch Events\. This rule sends a notification to the specified topic in Amazon Simple Notification Service \(Amazon SNS\) whenever an available instance in Amazon Elastic Compute Cloud \(Amazon EC2\) changes to a running state\. Also, related event information is logged to a log group in CloudWatch Events\.
