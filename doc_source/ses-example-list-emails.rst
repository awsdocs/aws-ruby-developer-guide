.. Copyright 2010-2018 Amazon.com, Inc. or its affiliates. All Rights Reserved.

   This work is licensed under a Creative Commons Attribution-NonCommercial-ShareAlike 4.0
   International License (the "License"). You may not use this file except in compliance with the
   License. A copy of the License is located at http://creativecommons.org/licenses/by-nc-sa/4.0/.

   This file is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND,
   either express or implied. See the License for the specific language governing permissions and
   limitations under the License.

.. _aws-ruby-sdk-ses-example-list-emails:

###################################
Listing Valid |SES| Email Addresses
###################################

.. meta::
    :description:
        Learn to list the Amazon SES email addresses through this AWS SDK for Ruby code example.
    :keywords: AWS SDK for Ruby code examples, Amazon SES

The following example demonstrates how to use the |sdk-ruby| to list the valid |SES|
email addresses.

.. literalinclude:: ./example_code/ses/ses_list_emails.rb
   :dedent: 0
   :language: ruby

See the `complete example <https://github.com/awsdocs/aws-doc-sdk-examples/blob/master/ruby/ses/ses_list_emails.rb>`_
on GitHub.
